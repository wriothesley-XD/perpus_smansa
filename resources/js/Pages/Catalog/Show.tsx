                ...
                <div className="grid gap-10 lg:grid-cols-[320px_1fr] lg:gap-14">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="relative w-full max-w-[280px]">
                            <div className="overflow-hidden rounded-2xl bg-white p-2.5 shadow-xl border border-slate-200/80">
                                <BookCover
                        ...
                        <div className="mt-6 w-full max-w-[280px] rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs">
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-medium text-slate-500">Status Fisik:</span>
                                {isAvailable ? (
                            ...
                            <div className="mt-4 space-y-2 border-t border-slate-100 pt-3 text-xs text-slate-600">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <strong className="text-emerald-700 font-bold">{book.available_copies_count ?? 0}</strong>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 space-y-2.5">
                                <button
                                    ...
                                    className="flex w-full items-center justify-center gap-2 rounded-full bg-[#152238] py-2.5 text-xs font-bold text-white shadow-xs transition hover:bg-[#0f172a]"
                                >
                                    <BookMarked size={14} />
                    ...
                    <div>
                        <div className="flex flex-wrap items-center gap-2">
                            {book.category?.name && (
                                <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-[#2699fb]">
                                    {book.category.name}
                                </span>
                            )}
                            {book.ddc_class?.code && (
                        ...
                        <h1 className="mt-4 font-display text-3xl font-extrabold tracking-tight text-[#0F172A] sm:text-4xl lg:text-5xl">
                            {book.title}
                        </h1>
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4">
                                ...
                                <strong className="mt-1 block text-xs text-[#0F172A] truncate">
                                    {book.publisher?.name ?? '-'}
                                </strong>
                            </div>
                            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4">
                            ...
                            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4">
                                ...
                                <strong className="mt-1 block font-mono text-xs text-[#0F172A] truncate">
                                    {book.isbn || '-'}
                                </strong>
                            </div>
                            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4">
                        ...
                        {/* Synopsis */}
                        <div className="mt-8">
                            <h2 className="font-display text-lg font-bold text-[#0F172A]">Sinopsis Buku</h2>
                                ...
                                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                                    {book.copies.map((copy, index) => {
                                        ...
                                        return (
                                            <div
                                                key={copy.id}
                                                className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white px-4 py-3 text-xs shadow-2xs"
                                            >
