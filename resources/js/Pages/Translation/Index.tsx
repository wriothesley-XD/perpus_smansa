            ...
            <div className="min-h-screen bg-white dark:bg-slate-900">
                ...
                <div className="mx-auto max-w-7xl px-6 py-14 sm:px-8">
                    ...
                    <div className="mx-auto mt-14 max-w-4xl rounded-3xl border border-slate-100 bg-[#fffdf7] p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-8">
                        <div className="flex items-center gap-3"><Globe className="text-[#0B4EA2]" size={22} /><div><h2 className="font-display text-xl font-bold text-[#0F172A] dark:text-white">Mulai menerjemahkan</h2><p className="text-sm text-slate-500 dark:text-slate-400">Tulis teks sumber, pilih bahasa tujuan, lalu lanjutkan proses penerjemahan.</p></div></div>
                        <div className="mt-6 grid gap-4 md:grid-cols-2"><textarea value={source} onChange={(e) => setSource(e.target.value)} rows={7} placeholder="Tulis teks bahasa Indonesia di sini..." className="w-full resize-none rounded-2xl border border-slate-200 bg-white p-4 text-sm outline-none focus:border-[#0B4EA2] dark:border-slate-700 dark:bg-slate-900 dark:text-white" /><div className="relative"><textarea value={translated} readOnly rows={7} placeholder="Hasil terjemahan akan tampil di sini..." className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-white" />{translated && <button type="button" onClick={() => navigator.clipboard?.writeText(translated)} className="absolute bottom-3 right-3 inline-flex items-center gap-1 rounded-lg bg-white px-3 py-1.5 text-xs font-bold text-slate-600 shadow-sm dark:bg-slate-800 dark:text-slate-200"><Copy size={13} /> Salin</button>}</div></div>
                        <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><select value={target} onChange={(e) => setTarget(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold dark:border-slate-700 dark:bg-slate-900 dark:text-white">{languages.map((lang) => <option key={lang.code} value={lang.code}>{lang.flag} {lang.name}</option>)}</select>{error && <span className="text-xs font-medium text-rose-600">{error}</span>}</div><button type="button" onClick={translate} disabled={processing || !source.trim()} className="rounded-xl bg-[#0B4EA2] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#083c7d] disabled:opacity-60">{processing ? 'Menerjemahkan...' : 'Terjemahkan'}</button></div>
                    </div>
                </div>
            </div>
        </SiteShell>
