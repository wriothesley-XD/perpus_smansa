    ...
    return (
        <SiteShell>
            ...
            <div className="bg-white dark:bg-slate-900 min-h-screen">
                {/* Header */}
                <div className="border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-[#152238] to-[#1d6fd6] px-6 py-16 text-white sm:px-8">
                    <div className="mx-auto max-w-7xl">
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                            {periods.map((item) => <button key={item.key} type="button" onClick={() => router.get('/ranking', { period: item.key }, { preserveScroll: true })} className={`rounded-full px-3 py-1.5 text-xs font-bold transition ${period === item.key ? 'bg-[#FFC533] text-[#152238]' : 'bg-white/10 text-white hover:bg-white/20'}`}>{item.label}</button>)}
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            <div className="mb-6 flex items-center gap-3">
                                <span className="grid size-10 place-items-center rounded-xl bg-[#E8F1F5] dark:bg-slate-800">
                                    <BookOpen size={20} className="text-[#152238] dark:text-blue-400" />
                                </span>
                                <div>
                            ...
                            <div className="space-y-3">
                                ...
                                {topBooks.map((book, idx) => (
                                    <Link
                                        ...
                                        className="flex items-center gap-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 hover:border-[#152238] dark:hover:border-blue-500 transition-all hover:-translate-y-0.5 hover:shadow-md"
                                    >
                                        ...
                                        <div className="shrink-0 text-right">
                                            <p className="text-lg font-extrabold text-[#152238] dark:text-blue-400">{book.loan_count ?? 0}</p>
                            ...
                            <div className="space-y-3">
                                ...
                                {topMembers.map((member, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="grid size-10 shrink-0 place-items-center rounded-full bg-[#152238] text-white font-bold text-sm">
                                            {member.initial_name}
                                        </div>
                                        <div className="flex-1 min-w-0">
