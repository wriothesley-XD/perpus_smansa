                ...
                <div className="border-b border-slate-100 dark:border-slate-800 bg-gradient-to-br from-rose-50 to-purple-50 dark:from-slate-800 dark:to-slate-900 px-6 py-14 sm:px-8">
                    <div className="mx-auto max-w-7xl">
                        <div className="flex items-center gap-3">
                            ...
                            <Link href="/contact?subject=Kirim%20Karya%20SMANSA" className="mt-5 inline-flex shrink-0 items-center gap-2 rounded-xl bg-[#152238] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#0f172a] sm:mt-0">Kirim Karya <span>↗</span></Link>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 sm:px-8">
                    <div className="mx-auto flex max-w-7xl flex-wrap gap-2 py-4">
                        ...
                        <button onClick={() => setFilter('category', null)}
                            className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${!filters.category ? 'bg-[#152238] text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                            Semua Kategori
                        </button>
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setFilter('category', cat)}
                                className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${filters.category === cat ? 'bg-[#152238] text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
