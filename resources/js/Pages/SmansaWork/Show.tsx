        ...
        <SiteShell>
            ...
            <div className="min-h-screen bg-white">
                    ...
                    <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-3.5 sm:px-8">
                        ...
                        {work.attachment_path && (
                            <a
                                ...
                                download
                                className="inline-flex items-center gap-1.5 rounded-full bg-[#152238] px-4 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-[#0f172a] transition-colors"
                            >
                                <Download size={13} />
                                <span>Unduh PDF</span>
                            </a>
                ...
                <div className="mx-auto max-w-3xl px-6 py-12 sm:px-8">
                    {/* Header */}
                    <div>
                        <span className="inline-block rounded-full bg-blue-50 px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-[#2699fb]">
                            {work.category_label ?? categoryLabel[work.category] ?? work.category}
                        </span>

                        <h1 className="mt-4 font-display text-3xl sm:text-4xl lg:text-5xl font-extrabold leading-[1.2] text-[#0F172A]">
                            {work.title}
                        </h1>

                        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-6">
                            <div className="flex items-center gap-3">
                                <span className="grid size-10 place-items-center rounded-full bg-slate-100 text-[#152238]">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <p className="text-sm font-bold text-[#0F172A]">{work.author_name}</p>
                                    ...
                                    {work.author_class && (
                                        <p className="text-xs text-slate-500">{work.author_class}</p>
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    <article className="mt-10 leading-relaxed">
                        <div className="whitespace-pre-line text-base sm:text-lg leading-[2] text-[#1E293B]">
                            {work.content}
                        </div>
                    </article>
                        ...
                        <div className="mt-20 border-t border-slate-200/80 pt-12">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <h2 className="mt-1 font-display text-xl sm:text-2xl font-bold text-[#0F172A]">
                            ...
                            <div className="mt-6 grid gap-4 sm:grid-cols-3">
                                {related.map((r) => (
                                    <Link
                                        ...
                                        className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition-all hover:-translate-y-0.5 hover:border-[#2699fb]/40 hover:shadow-md"
                                    >
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-[#2699fb]">
                                            {categoryLabel[r.category] ?? r.category}
                                        </span>
                                        <h3 className="mt-2 font-display text-sm font-bold leading-snug text-[#0F172A] group-hover:text-[#2699fb] line-clamp-2 transition-colors">
                                            {r.title}
                                        </h3>
                                        <p className="mt-3 text-xs text-slate-500">{r.author_name}</p>
                                    </Link>
                                ))}
                            </div>
                        </div>
